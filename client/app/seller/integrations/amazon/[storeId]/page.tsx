...
export default function AmazonStorePage() {
    ...
    const { data: logs, isLoading: isLogsLoading } = useSyncLogs(storeId, 'AMAZON');
    ...
    return (
        ...
        <div className="space-y-8 max-w-7xl mx-auto p-4 sm:p-6 md:p-8 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-[var(--border-subtle)]">
                <div className="flex items-start gap-5">
                    <div className="w-20 h-20 bg-[#FF9900]/10 rounded-2xl flex items-center justify-center border border-[#FF9900]/20 shadow-sm transition-transform hover:scale-105">
                        <img src="/logos/amazon.svg" alt="Amazon" className="w-12 h-12" />
                    </div>
                    <div className="space-y-1.5">
                        <div className="flex items-center gap-3 flex-wrap">
                            <h1 className="text-3xl font-bold text-[var(--text-primary)] tracking-tight">
                                {store.storeName || 'Amazon Store'}
                            </h1>
                            <Badge 
                        ...
                        <div className="flex flex-col gap-1">
                            <div className="text-sm font-medium text-[var(--text-secondary)] flex items-center gap-1.5">
                                <Badge variant="outline" className="text-[10px] uppercase font-bold">
                                    {(store.credentials as any)?.region || 'IN'}
                                </Badge>
                                <span className="opacity-70">Seller ID: {(store.credentials as any)?.sellerId?.substring(0, 4)}****</span>
                            </div>
                ...
                <div className="flex items-center gap-3">
                    <Button 
                        variant="outline" 
                        size="md"
                        onClick={() => router.push(`/seller/integrations/amazon/${storeId}/settings`)}
            ...
            <Card className="border-none bg-[var(--bg-secondary)] shadow-sm overflow-hidden">
                <CardHeader className="flex flex-row items-center justify-between px-6 py-5 border-b border-[var(--border-subtle)] bg-[var(--bg-primary)]/50">
                    ...
                    <Button 
                        variant="ghost" 
                        size="sm" 
                        onClick={() => router.push(`/seller/integrations/amazon/${storeId}/sync`)}
        ...
        <ConfirmDialog
            ...
            onConfirm={() => {
                disconnectStore({ integrationId: storeId, type: 'AMAZON' }, {
                    onSuccess: () => {
