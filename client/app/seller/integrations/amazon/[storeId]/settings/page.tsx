            ...
            <div className="flex items-center justify-between pb-6 border-b border-[var(--border-subtle)]">
                <div className="flex items-center gap-4">
                    <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => router.push(`/seller/integrations/amazon/${storeId}`)}
                        className="gap-2"
                    >
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-[var(--text-primary)]">Amazon Settings</h1>
                        <p className="text-sm text-[var(--text-secondary)] mt-1">{store?.storeName || 'Amazon Store'}</p>
                    </div>
                </div>
                <Button
                    onClick={handleSaveSettings}
                    disabled={isUpdating}
                    className="gap-2 bg-[#FF9900] hover:bg-[#CC7A00] text-white"
                >
                    {isUpdating ? (
            ...
            <Card className="bg-[#FF9900]/5 border-[#FF9900]/20">
                <CardContent className="p-4 flex items-start gap-3">
                    ...
                    <div className="space-y-1">
                        <p className="text-sm font-medium text-[var(--text-primary)]">Amazon SP-API Rate Limits</p>
                        <p className="text-xs text-[var(--text-secondary)]">
                            Amazon's Orders API has strict rate limits (1 request/minute). Avoid using frequencies below 15 minutes to prevent quota issues.
                        </p>
                    </div>
                </CardContent>
            </Card>
            ...
            <Card>
                ...
                <CardContent className="space-y-6">
                    <div className="space-y-2">
                        ...
                        />
                        <p className="text-xs text-[var(--text-muted)]">How often to check Amazon for new orders (respects API rate limits)</p>
                    </div>

                    <div className="flex items-center justify-between p-4 rounded-xl bg-[var(--bg-secondary)] border border-[var(--border-subtle)]">
                        <div className="flex-1">
                            <p className="text-sm font-medium text-[var(--text-primary)]">Auto-confirm shipment</p>
                            <p className="text-xs text-[var(--text-muted)] mt-1">Send tracking details back to Amazon automatically</p>
                        </div>
                        <Switch
                    ...
                    <div className="flex items-center justify-between p-4 rounded-xl bg-[var(--bg-secondary)] border border-[var(--border-subtle)]">
                        <div className="flex-1">
                            ...
                            <p className="text-xs text-[var(--text-muted)] mt-1">Sync last 7 days of orders (Amazon limits historical data access)</p>
                        </div>
                        <Switch
                ...
                <CardContent className="space-y-4">
                    ...
                    <div className="flex items-center justify-between p-4 rounded-xl bg-[var(--bg-secondary)] border border-[var(--border-subtle)]">
                        <div className="flex-1">
                            ...
                            <p className="text-xs text-[var(--text-muted)] mt-1">Alert when connection drops or credentials expire</p>
                        </div>
                        <Switch
