...
import type { SellerNavItem } from "@/src/components/seller/layout/Sidebar";
...
export function SellerSearch() {
    ...
    const allItems = useMemo(() => {
        return [
            ...sellerNavSections.flatMap(s => s.items),
            ...sellerAccountItems,
            ...sellerSupportItems
        ];
    }, []);

    // Group items by category (inferred from section or manual mapping)
    const groupedItems = useMemo(() => {
        ...
        const groups: Record<string, SellerNavItem[]> = {};

        filtered.forEach(item => {
            // Find which section this item falls into
            let category = 'Other';

            const foundSection = sellerNavSections.find(s => s.items.some(i => i.href === item.href));
            if (foundSection) {
                category = foundSection.title;
            } else if (sellerAccountItems.some(i => i.href === item.href)) {
                category = 'Account';
            } else if (sellerSupportItems.some(i => i.href === item.href)) {
                category = 'Support';
            }
    ...
    return (
        <>
            <button
                ...
                aria-label="Search seller pages (Cmd+K)"
            >
                                ...
                                {!query && (
                                    <div className="py-12 text-center">
                                        ...
                                        <p className="text-sm text-[var(--text-muted)]">
                                            Search for orders, shipments, settings, and more...
                                        </p>
                                    </div>
                                )}
