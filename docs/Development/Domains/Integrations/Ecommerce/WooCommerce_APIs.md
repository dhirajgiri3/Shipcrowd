# WooCommerce REST API Integration Guide for Helix
...
Helix integrates with WooCommerce stores via the REST API v3 to enable order management and fulfillment automation. This integration allows:
...
- **Read/Write** permission: Full access (required for Helix)
...
3. Configure:
   ```
   Name: Helix Order Created
   Status: Active
   Topic: Order created
   Delivery URL: https://Helix.com/webhooks/woocommerce/orders/created
...
Browser → Helix Server → WooCommerce API ✅  
...
3. Site name: `Helix-test-store`
4. PHP version: 8.0+
5. Click **"Add Site"**
6. Access: `http://Helix-test-store.local`
...
- [ ] **Firewall** configured to allow Helix IPs
