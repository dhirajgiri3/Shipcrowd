# Manual API Testing Plan for Helix Backend
...
3. **Setup Environments:**
   ...
   - Staging (https://staging.Helix.com)
...
1. **Weight Discrepancy Detection**
   ...
   - Velocity webhook → Helix creates weight dispute automatically
...
1. **COD Collection Timeline**
   ...
   - Day 5: Velocity deposits to Helix → Helix's bank account
   - Day 7: Helix transfers to seller → ₹97,500 (minus ₹500 platform fee)
...
4. **Multi-Channel Inventory Sync**
   - Helix order created → Inventory reserved → Sync to Shopify (reduce available qty)
   - Shopify order created → Webhook → Helix inventory reserved
...
2. **Order Auto-Sync (Webhook Flow)**
   ...
   - Helix receives webhook → Validates HMAC signature → Queues job
   - Background job processes webhook → Order created in Helix
   - Seller sees order in dashboard → Clicks "Ship" → Label generated

3. **Fulfillment Status Sync**
   - Order shipped in Helix → Update Shopify: "Order fulfilled"
   - Tracking number synced to Shopify → Customer sees tracking link

4. **Inventory Sync (Two-Way)**
   - Inventory updated in Helix → Sync to Shopify (update available qty)
   - Inventory updated in Shopify → Webhook → Update Helix inventory
...
3. **Product & Inventory Sync**
   - Two-way inventory sync: WooCommerce ↔ Helix
   - Product mapping: Link WooCommerce product to Helix SKU
...
3. **Product Mapping (Critical for Multi-Marketplace)**
   - Map Amazon ASIN → Helix SKU
   - Map Flipkart listing ID → Helix SKU
...
2. **Push Notifications (Web)**
   ...
   - Click notification → Opens Helix dashboard
