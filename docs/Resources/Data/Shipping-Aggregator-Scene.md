# Helix: Complete Shipping Aggregator Concept & Workflow
...
Riya runs "Riya's Jewels" on Instagram. She wants to start shipping orders using Helix. Here is her step-by-step flow:

### Step 1: Signup & Email Verification

- **Action**: Riya visits Helix.com and clicks "Sign Up"
...
- **Verification (DeepVue)**:
  - Helix sends these details to the DeepVue API
...
Helix works on a **Prepaid Wallet model** (like a Metro Card). We generally don't give "Credit" to small sellers because they might vanish after shipping.

- **Action**: Riya clicks "Recharge Wallet"
- **Payment**: She pays ₹500 via UPI/Card (Razorpay)
- **Result**: Her Helix Wallet balance shows ₹500
- Money is now in your (Helix's) bank account
...
- **System Action**: Helix asks Velocity: "Who can take 0.5kg from Bangalore to Mumbai?"
...
- **Backend**: Helix tells Velocity → Velocity tells Delhivery: "Go to Riya's house"
- **Physical Action**: A Delhivery boy comes to Riya's house
- **Scanning**: He scans the barcode on the packet
- **Status Update**: The order status in Helix changes from "Ready to Ship" to "Picked Up"
...
- Helix gets updates from Velocity every few hours (via Webhook) and updates Riya's dashboard
...
- **Action**: She clicks "Re-attempt" on Helix dashboard
...
- **Day 5**: Velocity deposits this money to Helix's Bank Account
- **Day 7 (Remittance Day)**:
  - Helix system sees that Riya is owed ₹1,000
  - Helix automatically transfers ₹1,000 to Riya's Bank Account (minus any fees)
  - Riya is happy
...
| **Helix (You)** | The Platform. You take money from Riya and pay Velocity. You maximize the gap (margin) |
...
**Setup**: Riya ships a packet. She declares weight as 0.5 kg. Helix charges her ₹40.
...
- **Notification**: Helix sends Riya an alert: "Weight discrepancy detected. Declared: 0.5kg, Actual: 1.2kg. Additional charge: ₹55"
- **Options**:
...
- Day 5: Velocity deposits to Helix's bank account
- Day 7: Helix transfers to Riya's account
...
**Process**:
1. **Template Download**: Riya downloads a CSV template from Helix
...
**How It Works**:
1. **OAuth Connection**: Riya clicks "Connect Shopify" in Helix
2. **Authorization**: She logs into Shopify and approves access
3. **Webhook Setup**: Helix registers a webhook with Shopify
4. **Auto-Sync**:
   - Customer places order on Shopify
   - Shopify sends webhook to Helix: "New order #1234"
   - Helix auto-creates the order
...
2. **Riya's Action**: Riya creates a "Return Order" in Helix dashboard
3. **Input Required**:
...
**Setup**: Helix has a sales team. Amit (salesperson) brings Riya as a customer.
...
**Setup**: Riya's customer receives SMS: "Track your order: https://Helix.com/track/ABC123". But the customer doesn't know Helix—they ordered from "Riya's Jewels".

**White-Label Tracking Solution**:

1. **Custom Domain**:
   - Riya sets up: tracking.riyasjewels.com
   - Helix provides: Subdomain or custom domain support
   - DNS: Riya points DNS to Helix servers

2. **Branding Customization**:
   - Logo: Riya uploads her logo (appears on tracking page)
   - Colors: Riya selects brand colors (matches her website)
   - Footer: "Powered by Helix" (small, at bottom) or completely white-label
...
**Setup**: Helix works with 5 courier partners (Delhivery, BlueDart, XpressBees, EcomExpress, DTDC). How do you know which one is best?
...
**Setup**: Helix sends webhook to Riya's server: "Order ABC123 is delivered". But Riya's server is down, so webhook fails.

**Webhook Retry Solution**:

**Initial Attempt**:
- Helix sends: `POST https://riyasjewels.com/webhook/order-update`
...
- Helix marks: Webhook failed, needs retry
...
**Setup**: Riya's developer builds an integration. They make 10,000 API calls per minute (accidentally, due to a bug). This crashes Helix's servers.
...
2. **E-way Bill Generation**:
   - System Integration: Helix connects to GST Portal API
...
2. **Documentation Required**:
   - Commercial Invoice: Generated by Helix
...
**Setup**: Helix receives order. Multiple couriers available. System must automatically pick best courier based on: price, delivery speed, performance, customer preferences, seller preferences, demand management.
...
**Setup**: Riya doesn't have ₹5,000 to recharge wallet. Can she get credit from Helix?
...
**Setup**: Riya's business grows. She now has 10 employees. How do they all access Helix with role-based permissions?
...
**Setup**: Riya has orders on Helix, Shopify, and Amazon. Total inventory: 100 units. She needs to prevent overselling.
...
**Setup**: High-value order (₹50,000). Helix wants to ensure package stays secure throughout transit.
...
**Setup**: Helix wants to attract sellers. Offer ₹500 discount coupon for first 100 orders.
...
This comprehensive system makes Helix a professional, scalable shipping aggregator platform that sellers trust and rely on.