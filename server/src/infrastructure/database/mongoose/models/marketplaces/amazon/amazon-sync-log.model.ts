 ...
 * AmazonSyncLog Model
 *
 * Tracks all Amazon SP-API synchronization operations for monitoring and debugging.
 *
 * Features:
...
export interface IAmazonSyncLog extends Document {
...
const AmazonSyncLogSchema = new Schema<IAmazonSyncLog>(
  {
    storeId: {
  ...
  {
    timestamps: true,
    collection: 'amazon_sync_logs',
  }
);

// Indexes
AmazonSyncLogSchema.index({ storeId: 1, syncType: 1, createdAt: -1 });
AmazonSyncLogSchema.index({ createdAt: -1 });
AmazonSyncLogSchema.index({ status: 1, syncType: 1 });

// Instance Methods

AmazonSyncLogSchema.methods.completeSync = async function (
  synced: number,
  skipped: number
): Promise<void> {
...
AmazonSyncLogSchema.methods.completeSyncWithErrors = async function (
  synced: number,
  skipped: number,
  failed: number,
  syncErrors: string[]
...
AmazonSyncLogSchema.methods.failSync = async function (error: string): Promise<void> {
  this.status = 'FAILED';
  this.endTime = new Date();
  this.duration = this.endTime.getTime() - this.startTime.getTime();
...
AmazonSyncLogSchema.statics.getRecentLogs = function (
  storeId: string,
  syncType?: string,
  limit: number = 10
) {
...
AmazonSyncLogSchema.statics.getSyncStats = async function (storeId: string, syncType?: string) {
...
AmazonSyncLogSchema.statics.cleanupOldLogs = async function (retentionDays: number = 90) {
...
const AmazonSyncLog = mongoose.model<IAmazonSyncLog>('AmazonSyncLog', AmazonSyncLogSchema);

export default AmazonSyncLog;
