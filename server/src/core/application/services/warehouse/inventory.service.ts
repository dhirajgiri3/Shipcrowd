/**
 * Inventory Service - Named Function Exports
 * Refactored from static class to match Week 1-4 patterns
 */
...
export const createInventory = async (data: ICreateInventoryDTO): Promise<IInventory> => {
...
export const updateInventory = async (data: IUpdateInventoryDTO): Promise<IInventory> => {
...
export const getInventoryBySKU = async (warehouseId: string, sku: string): Promise<IInventory | null> => {
    return Inventory.findOne({ warehouseId, sku });
};

export const getInventoryByBarcode = async (warehouseId: string, barcode: string): Promise<IInventory | null> => {
    return Inventory.findOne({ warehouseId, barcode });
};

export const getInventoryList = async (options: IInventoryQueryOptions): Promise<IPaginatedResult<IInventory>> => {
...
export const discontinueInventory = async (inventoryId: string): Promise<IInventory> => {
...
export const receiveStock = async (data: IReceiveStockDTO): Promise<{ inventory: IInventory; movement: IStockMovement }> => {
...
export const adjustStock = async (data: IAdjustStockDTO): Promise<{ inventory: IInventory; movement: IStockMovement }> => {
    const session = await mongoose.startSession();
    session.startTransaction();
...
export const reserveStock = async (data: IReserveStockDTO): Promise<IInventory> => {
...
export const releaseReservation = async (data: IReleaseReservationDTO): Promise<IInventory> => {
    const inventory = await Inventory.findById(data.inventoryId);
    if (!inventory) throw new AppError('Inventory not found', 'INVENTORY_NOT_FOUND', 404);
...
export const pickStock = async (data: IPickStockDTO): Promise<{ inventory: IInventory; movement: IStockMovement }> => {
    const inventory = await Inventory.findById(data.inventoryId);
    if (!inventory) throw new AppError('Inventory not found', 'INVENTORY_NOT_FOUND', 404);
...
export const transferStock = async (data: ITransferStockDTO): Promise<{ inventory: IInventory; movement: IStockMovement }> => {
    const inventory = await Inventory.findById(data.inventoryId);
    if (!inventory) throw new AppError('Inventory not found', 'INVENTORY_NOT_FOUND', 404);
...
export const markDamaged = async (data: IMarkDamagedDTO): Promise<{ inventory: IInventory; movement: IStockMovement }> => {
    const inventory = await Inventory.findById(data.inventoryId);
    if (!inventory) throw new AppError('Inventory not found', 'INVENTORY_NOT_FOUND', 404);
...
export const cycleCount = async (data: ICycleCountDTO): Promise<{ inventory: IInventory; movement?: IStockMovement }> => {
    const inventory = await Inventory.findById(data.inventoryId);
    if (!inventory) throw new AppError('Inventory not found', 'INVENTORY_NOT_FOUND', 404);
...
export const getLowStockAlerts = async (companyId: string): Promise<ILowStockAlert[]> => {
...
export const getMovements = async (options: IStockMovementQueryOptions): Promise<IPaginatedResult<IStockMovement>> => {
    const { page = 1, limit = 20 } = options;
    const filter: any = {};
    if (options.warehouseId) filter.warehouseId = options.warehouseId;
    if (options.companyId) filter.companyId = options.companyId;
...
export const getInventoryStats = async (warehouseId: string): Promise<IInventoryStats> => {
...
export const getMovementSummary = async (warehouseId: string, startDate: Date, endDate: Date): Promise<IMovementSummary> => {
...
export const generateMovementNumber = async (): Promise<string> => {
